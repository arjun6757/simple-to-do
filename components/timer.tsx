...
import { focusTask, unfocusTask } from "@/stores/slices/task";
...
export default function Timer({ active }: { active: boolean }) {
  ...
  return (
    ...
    >
      <div className="w-[65%] sm:h-1/2 sm:w-1/2 flex justify-center items-center relative">
        <CircularProgressbar
          value={percentage}
          // text={`${timeLeft}s`}
          // text={`${value}s`}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col gap-4 justify-center items-center text-gray-700">
          <span className="text-2xl font-semibold text-gray-600">Focus</span>
          ...
          <Select
            onValueChange={(id: string) => {
              if (focusingTask?.id === Number(id)) {
                const index = tasks.findIndex((t) => t.id === focusingTask.id);
                unfocusTask(tasks[index]); // dpesmt work
              }
          ...
          >
            <SelectTrigger
              icon={<ChevronRight className="size-4 text-current" />}
              size="sm"
              className="!p-0 !h-auto !gap-1 border-none justify-center shadow-none cursor-pointer group text-sm text-current max-w-40 truncate"
            >
              <SelectValue
                className={`${focusingTask ? "text-gray-700" : "text-gray-500"}`}
                placeholder={focusingTask ? focusingTask.task : "Select a task"}
              />
            </SelectTrigger>
            <SelectContent
              position="popper"
