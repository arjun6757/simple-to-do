...
export default async function Header() {
	...
	return (
		<Card className="py-3 border-none shadow-none w-full px-0 font-mono">
          ...
          {user ?
            (
              <form action={handleSignOut} className="flex items-center space-x-2">
                {/*<p>{user.email?.split("@")[0]}</p>*/}
              <div className="flex gap-2 items-center rounded-md px-2 py-1 bg-[#f0f0f0] font-sans">
              	<img referrerPolicy="no-referrer" src={user.user_metadata?.avatar_url} alt="avatar" className="w-8" />
